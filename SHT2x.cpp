...
SHT2x::SHT2x(i2c_master_bus_handle_t _bus_handle)
    : _bus_handle(_bus_handle), _lastRead(0), _lastRequest(0) {
  _lastRead = 0;
  _lastRequest = 0;
...
esp_err_t SHT2x::begin() {

  i2c_device_config_t dev_cfg = {
      .dev_addr_length = I2C_ADDR_BIT_LEN_7,
      .device_address = SHT2x_ADDRESS,
      .scl_speed_hz = 100000,
  };
...
bool SHT2x::isConnected() {
    uint8_t dummy;
    esp_err_t ret = i2c_master_transmit_receive(_dev_handle, NULL, 0, &dummy, 1, 100);
    return (ret == ESP_OK);
}
...
bool SHT2x::requestTemperature() {
  writeCmd(SHT2x_GET_TEMPERATURE_NO_HOLD);  _lastRequest = esp_timer_get_time() / 1000;
...
bool SHT2x::reqTempReady() {
  ...
  int64_t waiting = (esp_timer_get_time() / 1000) - _lastRequest;
  //  table 7
  if (waiting < 11)
    return false;
  if (_resolution == 3)
...
bool SHT2x::requestReady() { return (reqTempReady() || reqHumReady()); }
...
uint32_t SHT2x::lastRequest() { return _lastRequest; }

uint8_t SHT2x::getRequestType() { return _requestType; }
...
uint16_t SHT2x::getRawTemperature() { return _rawTemperature; }

uint16_t SHT2x::getRawHumidity() { return _rawHumidity; }
...
uint8_t SHT2x::getStatus() { return _status; }

uint32_t SHT2x::lastRead() { return _lastRead; }
...
uint8_t SHT2x::getHeatTimeout() { return _heatTimeout; }

bool SHT2x::heatOn() {
  if (isHeaterOn())
    return true;
  if ((_heaterStop > 0) && ((esp_timer_get_time() / 1000) - _heaterStop < SHT2x_HEATER_TIMEOUT)) {
...
uint8_t SHT2x::getResolution() { return _resolution; };
...
bool SHT2x::writeCmd(uint8_t cmd) {
    esp_err_t ret = i2c_master_transmit(_dev_handle, &cmd, 1, -1);
    if (ret != ESP_OK) {
...
bool SHT2x::writeCmd(uint8_t cmd, uint8_t value) {
    ...
    esp_err_t ret = i2c_master_transmit(_dev_handle, data, 2, -1);
    if (ret != ESP_OK) {
        _error = SHT2x_ERR_WRITECMD;
        return false;
    }
...
bool SHT2x::readBytes(uint8_t n, uint8_t* val, uint8_t maxDuration) {
    esp_err_t ret = i2c_master_receive(_dev_handle, val, n, -1);
    if (ret != ESP_OK) {
        _error = SHT2x_ERR_READBYTES;
        return false;
    }
...
//  DERIVED SHT
//
SHT20::SHT20(i2c_master_bus_handle_t _bus_handle) : SHT2x(_bus_handle) {}

SHT21::SHT21(i2c_master_bus_handle_t _bus_handle) : SHT2x(_bus_handle) {}

SHT25::SHT25(i2c_master_bus_handle_t _bus_handle) : SHT2x(_bus_handle) {}

////////////////////////////////////////////////////////
//
//  DERIVED HTU
//
HTU20::HTU20(i2c_master_bus_handle_t _bus_handle) : SHT2x(_bus_handle) {}

HTU21::HTU21(i2c_master_bus_handle_t _bus_handle) : SHT2x(_bus_handle) {}

////////////////////////////////////////////////////////
//
//  DERIVED Si70xx
//
Si7013::Si7013(i2c_master_bus_handle_t _bus_handle) : SHT2x(_bus_handle) {}

Si7020::Si7020(i2c_master_bus_handle_t _bus_handle) : SHT2x(_bus_handle) {}

Si7021::Si7021(i2c_master_bus_handle_t _bus_handle) : SHT2x(_bus_handle) {}

////////////////////////////////////////////////////////
//
//  DERIVED Si70xx
//
GY21::GY21(i2c_master_bus_handle_t _bus_handle) : SHT2x(_bus_handle) {}
